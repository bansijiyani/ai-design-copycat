...
function ProductsPageContent() {
  ...
  const setCategory = (c: string) => router.push(c === "all" ? "/products" : `/products?category=${c}`);
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="container mx-auto px-4 py-4 text-sm text-muted-foreground">
        <Link href="/" className="hover:text-gold">Home</Link> / <span className="text-foreground capitalize">{active}</span>
      </div>
...
export default function ProductsPage() {
  return (
    <Suspense fallback={<div className="min-h-screen bg-background flex items-center justify-center">Loading products...</div>}>
      <ProductsPageContent />
    </Suspense>
  );
}
