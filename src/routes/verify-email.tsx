...
function VerifyEmail() {
  ...
  const handleSendOtp = async () => {
    ...
    setSending(true);
    try {
      await sendOtpEmail({ data: { email: user.email, type: "signup" } });
  ...
  const handleVerify = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // We force reload to refresh session user metadata
      ...
      <main className="flex-grow flex items-center justify-center p-4">
        <div className="w-full max-w-md bg-white p-8 rounded shadow-sm border border-border">
          ...
          <p className="text-muted-foreground text-sm mb-6">
            ...
            Click below to send a code to <strong>{user.email}</strong>.
          </p>
